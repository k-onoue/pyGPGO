...
def generate_noise(size, noise_type="normal", df=1):
    ...
    if noise_type == "normal":
        ...
        return torch.distributions.StudentT(df).sample(size)
...
class TestFunction:
    def __init__(
        ...
        self.search_space: torch.Tensor = torch.tensor([])  # Set the search space externally
...
class SinusoidalSynthetic(TestFunction):
    r"""
    Computes the function f(x) = sin(x) for a given torch input x, with independent noise added.

    Args:
        x (torch.Tensor): Input array of shape (N, 1) where N is the number of data points.
                          If the input is (N,), it will be automatically reshaped to (N, 1).

    Returns:
        torch.Tensor: Output array of shape (N, 1) representing the computed values of f(x).
    """
...
class Hartmann6(TestFunction):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        alpha = torch.tensor([1.00, 1.20, 3.00, 3.20])
        A = torch.tensor(
            [
        ...
        outer_sum = 0
        for i in range(4):
            inner_sum = torch.sum(A[i] * (x - P[i]) ** 2, axis=1)
...
if __name__ == "__main__":
    ...
    x_values = torch.linspace(5, 10, 100).reshape(-1, 1)
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x_values.flatten().numpy(), y=y_values.flatten().numpy(), mode='markers', name='With Noise'))
    fig.update_layout(
